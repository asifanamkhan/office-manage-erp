...
@endpush
<form class="add-bank-account" enctype="multipart/form-data" action="{{ route('admin.employee-bank-account.store') }}"
    method="POST">
    @csrf
    <div class="row">
        ...
        <div class="form-group col-12 col-sm-12 col-md-6 mb-2">
            <label for="routing_no"><b>Routing Number</b></label>
                <input type="text" name="routing_no" id="routing_no"class="form-control @error('routing_no') is-invalid @enderror" value="{{ old('routing_no') }}"placeholder="Enter Routing Number">
                    @error('routing_no')
                        <span class="invalid-feedback" role="alert">
                            <strong>{{ $message }}</strong>
                        </span>
        ...
        <div class="form-group col-12 mb-2">
            <label for="descriptions"><b>Description</b></label>
                <textarea name="descriptions" id="descriptions" rows="3"class="form-control descriptions" value="{{ old('description') }}"placeholder="Description..."></textarea>
                    @error('description')
                        <span class="invalid-feedback" role="alert">
                            <strong>{{ $message }}</strong>
                        </span>
    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            var dTable = $('#bank_account_table').DataTable({
                ...
                ajax: {
                    url: "{{ route('admin.employee-bank-account.show', $employee->id) }}",
                    type: "get"
                },
                columns: [
        ...
        function clientBankAccountDelete(id) {
            var url = '{{route('admin.employee-bank-account.destroy', ':id')}}';
            $.ajax({
                type: "DELETE",
        ...
        function statusChange(id) {
            var url = '{{ route("admin.employee.bank.update.status",":id") }}';
            $.ajax({
                type: "GET",
                url: url.replace(':id', id),
                success: function (resp) {
