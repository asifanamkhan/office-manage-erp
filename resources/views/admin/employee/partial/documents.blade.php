...
@endpush
<form class="add-employee-document" enctype="multipart/form-data" action="{{ route('admin.employees-document.store') }}"
      method="POST">
    @csrf
    <div class="row">
    ...
    <script>
        $(document).ready(function () {
            ...
            var dTable = $('#document_table').DataTable({
                ...
                ajax: {
                    url: "{{ route('admin.employees-document.show', $employee->id) }}",
                    type: "get"
                },
                columns: [
        ...
        function documentDelete(id) {
            var url = '{{ route('admin.employees-document.destroy', ':id') }}';
            $.ajax({
                type: "DELETE",
