...
@section('breadcrumb')
    <nav aria-label="breadcrumb" class="d-flex align-items-center justify-content-between" style="width: 100%">
        ...
        <a class="btn btn-sm btn-secondary text-white" href="{{ route('admin.crm.client-comment.index',$comment->id) }}">
            Back to list
        </a>
    </nav>
@endsection
        ...
        <div class="card mb-4">
            <div class="card-body">
                <form class="add-client-document" enctype="multipart/form-data"
                    action="{{ route('admin.crm.comment.update',['id' => $comment->id, 'parameter' => $parameter]) }}" method="POST">
                    @csrf
                    @method('PUT')
                    <div class="row">
                        ...
                        <div class="form-group col-12 col-sm-12 col-md-12 mb-2">
                            ...
                            <textarea name="comment" id="comment" rows="5" cols="40"class="form-control "placeholder="Enter Comment...">{{$comment->comment}}</textarea>
                        </div>
