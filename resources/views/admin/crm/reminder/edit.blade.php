...
@section('breadcrumb')
    <nav aria-label="breadcrumb" class="d-flex align-items-center justify-content-between" style="width: 100%">
        ...
        <a class="btn btn-sm btn-success text-white" href="{{ route('admin.crm.client.index') }}">
            Back to list
        </a>
    </nav>
@endsection
        ...
        <div class="card mb-4">
            <div class="card-body">
                <form class="add-client-document" enctype="multipart/form-data"
                    action="{{ route('admin.crm.reminder.update',['id' => $reminder->id, 'parameter' => $parameter]) }}" method="POST">
                    @csrf
                    @method('PUT')
                    <div class="row">
                            ...
                            <select name="client_id" id="client_id"class="form-select select2">
                                <option value="" >--Select Client--</option>
                                    @foreach ($clients as $client)
                                        <option value="{{$client->id}}" {{ $reminder->client_id == $client->id ? 'selected' : ''}}  >{{$client->name}}</option>
                                    @endforeach
                            </select>
                            @error('client_id')
                        ...
                        <div class="form-group col-12 col-sm-12 col-md-6 mb-2">
                            ...
                            <input type="date" id="date" class="form-control " name="date" value="{{$reminder->date}}">
                            @error('date')
                                <span class="invalid-feedback" role="alert">
                                    <strong>{{ $message }}</strong>
                                </span>
                        ...
                        <div class="form-group col-12 col-sm-12 col-md-6 mb-2">
                            ...
                            <input type="time" id="time" class="form-control " name="time" value="{{$reminder->time}}">
                            @error('time')
                                <span class="invalid-feedback" role="alert">
                                    <strong>{{ $message }}</strong>
                                </span>
                        ...
                        <div class="form-group col-12 col-sm-12 col-md-12 mb-2">
                            ...
                            <textarea name="reminder_note" id="reminder_note" rows="10" cols="40"class="form-control "
                                value="{{ old('reminder_note') }}"placeholder="Enter Reminder Note...">{{$reminder->reminder_note}}</textarea>
                            @error('reminder_note')
                                <span class="invalid-feedback" role="alert">
                                    <strong>{{ $message }}</strong>
                                </span>
