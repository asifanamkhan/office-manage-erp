...
@section('breadcrumb')
    <nav aria-label="breadcrumb" class="d-flex align-items-center justify-content-between" style="width: 100%">
        ...
        <a class="btn btn-sm btn-success text-white" href="{{ url('admin/raw-material/create') }}">
    ...
    <script>
        $(document).ready(function () {
            ...
            var dTable = $('#table').DataTable({
                ...
                ajax: {
                    url: "{{route('admin.raw-material.index')}}",
                    type: "get"
                },
                columns: [
                    {data: "DT_RowIndex",      name: "DT_RowIndex",       orderable: false,  searchable: false},
                    {data: 'name',             name: 'name',    orderable: true,   searchable: true},
                    {data: 'wirehouse_name',   name: 'wirehouse_name',    orderable: true,   searchable: true},
        ...
        function deleteItem(id) {
            var url = '{{ route("admin.raw-material.destroy",":id") }}';
            $.ajax({
                type: "DELETE",
        ...
        function statusChange(id) {
            var url = '{{ route("admin.raw-material.update.status",":id") }}';
            $.ajax({
                type: "GET",
                url: url.replace(':id', id),
                success: function (resp) {
