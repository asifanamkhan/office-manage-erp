...
@endpush
<form class="add-client-document" enctype="multipart/form-data" action="{{ route('admin.project.document.store') }}"
      method="POST">
    @csrf
    <div class="row">
    ...
    <script>
        $(document).ready(function () {
            ...
            var dTable = $('#document_table').DataTable({
                ...
                ajax: {
                    url: "{{ route('admin.project.document.show', $project->id) }}",
                    type: "get"
                },
                columns: [
        ...
        function documentDelete(id) {
            var url = '{{ route('admin.project.document.destroy', ':id') }}';
            $.ajax({
                type: "DELETE",
