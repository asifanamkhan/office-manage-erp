...
@section('breadcrumb')
    <nav aria-label="breadcrumb" class="d-flex align-items-center justify-content-between" style="width: 100%">
        ...
        data-coreui-target="#addCountryModal">
...
@section('content')
                    ...
                    <form class="add-country" enctype="multipart/form-data">
                        <div class="row">
                            <div class="form-group col-12 mb-2">
                                ...
                                <input type="text" name="country_name" id="country_name"class="form-control @error('country_name') is-invalid @enderror"value="{{ old('country_name') }}" placeholder="Country Name">
                                @error('country_name')
    ...
    <div class="modal fade" id="editCountryModal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="exampleModalLabel"><b>Edit Country</b></h5>
                    <button type="button" class="btn-close" data-coreui-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body">
                    <form class="edit-country" enctype="multipart/form-data">
                        <div class="row">
                            <div class="form-group col-12 mb-2">
                                <label for="country_name"><b>Country Name</b><span class="text-danger">*</span></label>
                                <input type="text" name="country_name" id="edit_country_name"class="form-control @error('country_name') is-invalid @enderror"value="{{ old('country_name') }}" placeholder="Country Name">
                                @error('country_name')
                                    <span class="invalid-feedback" role="alert">
                                        <strong>{{ $message }}</strong>
                                    </span>
    ...
    <script>
        $(document).ready(function () {
            ...
            var dTable = $('#table').DataTable({
                ...
                ajax: {
                    url: "{{route('admin.address.country.index')}}",
                    type: "get"
                },
                columns: [
        ...
        $(document).ready(function($){
            $(".add-country").submit(function(event) {
                ...
                var post_url = '{{ route("admin.address.country.store") }}'; //get form action url
        ...
        function getSelectedUserData(id){
            var url = '{{ route("admin.address.country.edit",":id") }}';
            $.ajax({
                type: "GET",
                url: url.replace(':id', id),
                success: function(resp) {
                    console.log(resp);
                    $('#editCountryModal #edit_country_name').val(resp.name);
                    $('#editCountryModal #country_id').val(resp.id);
        ...
        $(document).ready(function($){
            $(".edit-country").submit(function(event) {
                event.preventDefault(); //prevent default action
                var url = '{{ route("admin.address.country.update",":id") }}'; //get form action url
        ...
        function deleteItem(id) {
            var url = '{{ route("admin.address.country.destroy",":id") }}';
            $.ajax({
                type: "DELETE",
                url: url.replace(':id', id),
                success: function (resp) {
