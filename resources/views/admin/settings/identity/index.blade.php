...
@section('breadcrumb')
    <nav aria-label="breadcrumb" class="d-flex align-items-center justify-content-between" style="width: 100%">
        ...
        <a class="btn btn-sm btn-success text-white" href="{{ route('admin.settings.identity.create') }}">
    ...
    <script>
        $(document).ready(function () {
            ...
            var dTable = $('#table').DataTable({
                ...
                ajax: {
                    url: "{{route('admin.settings.identity.index')}}",
                    type: "get"
                },
                columns: [
        ...
        function deleteItem(id) {
            var url = '{{ route("admin.settings.identity.destroy",":id") }}';
            $.ajax({
                type: "DELETE",
        ...
        function statusChange(id) {
            var url = '{{ route("admin.settings.identity.update.status",":id") }}';
            $.ajax({
                type: "GET",
                url: url.replace(':id', id),
                success: function (resp) {
