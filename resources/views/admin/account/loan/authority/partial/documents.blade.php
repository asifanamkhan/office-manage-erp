...
@endpush
<form class="add-client-document" enctype="multipart/form-data" action="{{ route('admin.loan.authority.document.store') }}"
    method="POST">
    @csrf
    <div class="row">
    ...
    <script>
        $(document).ready(function() {
            ...
            var dTable = $('#document_table').DataTable({
                ...
                ajax: {
                    url: "{{ route('admin.loan.authority.document.show', $authority->id) }}",
                    type: "get"
                },
                columns: [
        ...
        function documentDelete(id) {
            var url = '{{ route('admin.loan.authority.document.destroy', ':id') }}';
            $.ajax({
                type: "DELETE",
