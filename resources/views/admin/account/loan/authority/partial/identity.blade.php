...
@endpush
<form class="add-client-identity" enctype="multipart/form-data" action="{{route('admin.loan.authority.identity.store')}}" method="POST">
    @csrf
    <div class="row">
            ...
            <select name="id_type_id" id="id_type_id" class="form-control" required>
                ...
                @foreach ($identities as $identity)
                    <option value="{{$identity->id}}"
                        @foreach ($authorityIdentity as $Identity)
        ...
        <div class="form-group col-12 col-sm-12 col-md-6 mb-2">
            ...
            <input type="number" name="id_no" id="id_no"class="form-control @error('id_no') is-invalid @enderror"value="{{ old('id_no') }}" placeholder="Enter Identity Number" required>
            @error('id_no')
                <span class="invalid-feedback" role="alert">
                    <strong>{{ $message }}</strong>
                </span>
        ...
        <input type="hidden" name="authority_id" id="authority_id" value="{{ $authority->id}}">
        ...
        <script>
            $(document).ready(function () {
                ...
                var dTable = $('#table').DataTable({
                    ...
                    ajax: {
                        url: "{{route('admin.loan.authority.identity.show', $authority->id)}}",
                        type: "get"
                    },
                    columns: [
            ...
            function deleteItem(id) {
                var url = '{{ route("admin.loan.authority.identity.destroy",":id") }}';
                $.ajax({
                    type: "DELETE",
