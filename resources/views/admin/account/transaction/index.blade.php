...
@section('breadcrumb')
    <nav aria-label="breadcrumb" class="d-flex align-items-center justify-content-between" style="width: 100%">
        ...
        <a class="btn btn-sm btn-success text-white" href="{{ route('admin.account.transaction.create') }}">
    ...
    <script>
        $(document).ready(function() {
            ...
            var dTable = $('#table').DataTable({
                ...
                ajax: {
                    url: "{{route('admin.account.transaction.index')}}",
                    type: "get"
                },
                columns: [
        ...
        // delete Confirm
        function showTransactionDeleteConfirm(id) {
            ...
            swal({
                ...
                if (willDelete) {
                    transactiondeleteItem(id);
                }
            });
        };
        // Delete Button
        function transactiondeleteItem(id) {
            var url = '{{ route('admin.account.transaction.destroy', ':id') }}';
            $.ajax({
                type: "DELETE",
        ...
        function statusChange(id) {
            var url = '{{ route('admin.account.transaction.update.status', ':id') }}';
            $.ajax({
                type: "GET",
                url: url.replace(':id', id),
                success: function(resp) {
