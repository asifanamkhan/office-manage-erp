...
@endpush
<form class="add-client-document" enctype="multipart/form-data" action="{{ route('admin.investor.document.store') }}"
    method="POST">
    @csrf
    <div class="row">
    ...
    <script>
        $(document).ready(function() {
            ...
            var dTable = $('#document_table').DataTable({
                ...
                ajax: {
                    url: "{{ route('admin.investor.document.show', $investor->id) }}",
                    type: "get"
                },
                columns: [
        ...
        function documentDelete(id) {
            var url = '{{ route('admin.investor.document.destroy', ':id') }}';
            $.ajax({
                type: "DELETE",
