                    ...
                    <div class="table-responsive">
                        <table class="table border mb-0" id="table">
                            ...
                            <tr class="align-middle table">
                                <tr>
                                    <th width="5%">SL#</th>
                                    <th width="20%">Cash-in Account Info</th>
                                    <th width="20%">Cash-out Account Info</th>
    ...
    <script>
        $(document).ready(function () {
            ...
            var dTable = $('#table').DataTable({
                ...
                ajax: {
                    url: "{{route('admin.account.fund-transfer.index')}}",
                    type: "get"
                },
                columns: [
                    {data: "DT_RowIndex", name: "DT_RowIndex", orderable: false, searchable: false},
                    {data: 'cash_in', name: 'cash_in', orderable: true, searchable: true},
                    {data: 'cash_out', name: 'cash_out', orderable: true, searchable: true},
                    {data: 'amount', name: 'amount', orderable: true, searchable: true},
                    {data: 'date', name: 'date', orderable: true, searchable: true},
                    //only those have manage_user permission will get access
                    {data: 'created_by.name', name: 'created_by.name', orderable: true, searchable: true},
        ...
        function deleteItem(id) {
            var url = '{{ route("admin.account.fund-transfer.destroy",":id") }}';
            $.ajax({
                type: "DELETE",
        ...
        function statusChange(id) {
            var url = '{{ route("admin.account.bank.update.status",":id") }}';
            $.ajax({
                type: "GET",
                url: url.replace(':id', id),
                success: function (resp) {
